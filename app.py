...
def serve_report():
    if request.method == 'POST':
        ...
        report = compliance_run("/app/", AUDIT_DATABASE, Report_Files, Json_Files)
        ####################################
        ...
        ####################
        return render_template('report.html', message=message, reports=contents.split("total")[1], debug=Json_Files)
